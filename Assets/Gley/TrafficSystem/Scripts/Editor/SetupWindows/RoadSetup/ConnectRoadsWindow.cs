...
namespace Gley.TrafficSystem.Editor
{
    public class ConnectRoadsWindow : ConnectRoadsWindowBase
    {
        ...
        {
            roadsOfInterest = roadDrawer.ShowAllRoads(MoveTools.None, roadColors.roadColor, roadColors.anchorPointColor, roadColors.controlPointColor, roadColors.textColor,
                true, save.viewRoadsSettings.viewWaypoints, save.viewRoadsSettings.viewLaneChanges, roadColors.laneColor, roadColors.waypointColor, roadColors.disconnectedColor, roadColors.laneChangeColor);

             MakeClickConnection(roadsOfInterest, roadConnections.ConnectionPools, roadColors.connectorLaneColor, roadColors.anchorPointColor,
            ...
            //{
            //    roadDrawer.DrawPath(allRoads[i], MoveTools.None, roadColors.roadColor, roadColors.anchorPointColor, roadColors.controlPointColor, roadColors.textColor, true);
        ...
        protected override void DrawConnections()
        {
            for (int i = 0; i < roadConnections.ConnectionPools.Count; i++)
            {
                for (int j = 0; j < roadConnections.ConnectionPools[i].GetNrOfConnections(); j++)
                {
                    ...
                    if (GleyUtilities.IsPointInViewWithValidation((roadConnections.ConnectionPools[i]).GetInConnector(j).transform.position) ||
                       GleyUtilities.IsPointInViewWithValidation((roadConnections.ConnectionPools[i]).GetOutConnector<WaypointSettingsBase>(j).transform.position))
                    {
                        EditorGUILayout.BeginHorizontal();
